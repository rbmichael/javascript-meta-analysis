...
$(function() {
	...
	$('#add').click(function() {
		...
		$('#studies').append("<div>" + "M<sub>1</sub>: <input type=\"number\" value=\"\"></input>" + "SD<sub>1</sub>: <input type=\"number\" value=\"\"></input>" + "M<sub>2</sub>: <input type=\"number\" value=\"\"></input>" + "SD<sub>2</sub>: <input type=\"number\" value=\"\"></input>" + "N: <input type=\"number\" value=\"\"></input>" + "<button class=\"remove\">remove</button><br /></div>");
		...
		if (rows.length > 1) { // if there's a minimum of 2 studies, populate the form
			...
			for (var i = 0; i < data.length; i++) {
				...
				$('#studies div:last input').each(function(index) { // populate it with data
					switch (index) {
						case 0: // insert mean 1
							$(this).val(data[i][0]);
							break;
						case 1: // insert standard deviation 1
							$(this).val(data[i][1]);
							break;
						case 2: // insert mean 2
							$(this).val(data[i][2]);
							break;
						case 3: // insert standard deviation 2
							$(this).val(data[i][3]);
							break;
						case 4: // insert N
							$(this).val(data[i][4]);
	...
	$('#run').click(function() {
		/* step 1: check data entry, proceed if OK, else return error
			- must have 2+ studies, with each having 2: m, sd > 0; and an n >= 2
		*/
		var dataOK = checkData();
		if (!dataOK) { alert("Error with data entry. Each study must have two valid Means and Standard Deviations, and an N >= 2."); return; }
...
// data entry check: minimum 2 studies, each with 2 * M = a number, 2 * SD > 0 and 1 * N >= 2
function checkData() {
...
function getData() {
	...
	var studyData = { m1:null, sd1:null, m2:null, sd2:null, n:null, mDiff:null, pooledSD:null, varDiff:null, moeDiff:null, weight:null, t:null, p:null }; // an object for study data
	// loop through the studies
	$('#studies div').each(function(index) {
		// get Ms, SDs, and N
		$(this).children('input').each(function(index) {
			switch(index) {
				case 0:
					studyData.m1 = Number($(this).val()); // Mean 1
					break;
				case 1:
					studyData.sd1 = Number($(this).val()); // SD 1
					break;
				case 2:
					studyData.m2 = Number($(this).val()); // Mean 2
					break;
				case 3:
					studyData.sd2 = Number($(this).val()); // SD 2
					break;
				case 4:
		...
		// how to get the necessary values with only summary data?
		studyData.sdDiff = Math.sqrt((studyData.mDiff*studyData.mDiff) / (studyData.n-1)); // calculate sd of the difference
		console.log(studyData.sdDiff / Math.sqrt(studyData.n)); // standard error!?!?
